...
Fax : +6221 2995 1001</div>
      ...
      <div id="project">
        <div><span>CLIENT</span> {{ $po->ro->user->name }}</div>
        <div><span>DEPARTMENT</span> {{ $po->ro->department->d_name }}</div>
        <div><span>VENDOR</span> {{ $po->vendor->v_name }}</div>
        <div><span>EMAIL</span> <a href="mailto:john@example.com">{{ $po->ro->user->email }}</a></div>
        <div><span>PO DATE</span> {{ date("d F Y",strtotime($po->created_at)) }}</div>
      </div>
    </header>
    <main>
      <table>
        ...
        @foreach($po->detail_po as $item)
          <tr>
            ...
            <td class="qty">{{ number_format($item->product->price) }}</td>
            <td class="qty">{{ number_format($item->qty) }}</td>
            <td class="qty">{{ number_format($item->qty * $item->product->price) }}</td>
          </tr>
          <?php $total +=$item->qty;
